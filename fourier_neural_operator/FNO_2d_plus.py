...
class SpectralConv2d(tf.keras.layers.Layer):
    ...
    def __init__(self,
            in_channels:int,
            out_channels:int,
            modes:int or tuple or list,
            first_channel_unchanged:bool,
            HW_out_imposed:int or tuple or list,
            freq_mix_size:int,
            channel_last:bool):
        super().__init__()
        ...
        self.modes1,self.modes2 = one_or_two(modes)  #Number of Fourier modes to multiply, at most floor(N/2) + 1
        self.first_channel_unchanged=first_channel_unchanged

        self.H_out_imposed,self.W_out_imposed = one_or_two(HW_out_imposed)

        self.freq_mix_size=freq_mix_size
        self.channel_last=channel_last

        self.weights1  = self.get_complex_weights()
        self.weights2  = self.get_complex_weights()
    ...
    def get_complex_weights_classic(self):
        ...
        real = tf.random.uniform([self.in_channels, self.out_channels, self.modes1,self.modes2])
        img  = tf.random.uniform([self.in_channels, self.out_channels, self.modes1,self.modes2])
        return tf.Variable(tf.complex(real,img)*scale)

    def get_complex_weights_for_first_channel_unchanged(self):
        scale = (1 / (self.in_channels*self.out_channels))
        real = np.random.uniform(0.,1,size=[self.in_channels, self.out_channels, self.modes1,self.modes2]).astype(np.float32)
        img =  np.random.uniform(0.,1,size=[self.in_channels, self.out_channels, self.modes1,self.modes2]).astype(np.float32)
        real[:,0,:,:]=1
        img[:,0,:,:]=0.
    ...
    def freq_conv(self,B,kernel):
        ...
        if len(list_GPU)==0:
            ...
            B=tf.nn.conv1d(B, kernel, stride=1, padding="VALID", data_format="NHWC")
            return tf.transpose(B,[0,2,3,1])
        else:
            #print("GPU")
            return tf.nn.conv1d(B,kernel,stride=1,padding="VALID",data_format="NCHW")


    def get_kernel_for_freq_conv(self):
        kernel=np.zeros([self.freq_mix_size ,self.freq_mix_size ,self.out_channels,self.out_channels]).astype(np.float32)
        kernel[0,0,:,:]=1.
    ...
    def call(self, A):
        ...
        _,c,H,W=A.shape
        if self.H_out_imposed is None:
            H_out=H
            W_out=W
        else:
            H_out,W_out=self.H_out_imposed,self.W_out_imposed

        assert H_out>=2*self.modes1,f"H_out={H_out} must be twice bigger than modes1={self.modes1}"
        assert W_out>=2*self.modes2,f"H_out={H_out} must be twice bigger than modes1={self.modes2}"
        ...
        out_ft_corner_SO =tf.pad(out_ft_corner_SO,[[0,0],[0,0],[0,H_out - self.modes1], [0,W_out - self.modes2]])
        out_ft_corner_NO =tf.pad(out_ft_corner_NO,[[0,0],[0,0],[H_out  - self.modes1,0],[0,W_out - self.modes2]])
        out_ft=out_ft_corner_SO+out_ft_corner_NO
        #Return to physical space
        A = tf.signal.irfft2d(out_ft, fft_length=[H_out,W_out]) #fft_length=
...
def test_spectral_conv_2D():
    ...
    nx=11 #doit être plus grand que modes
    ny=13
    ...
    assert Y.shape == (batch_size,nx ,ny,out_channels), f"Y.shape:{Y.shape} must be equal to (batch_size,H,W,out_channels):{(batch_size,nx,ny, out_channels)} "
    print("X.shape", X.shape)
    print("Y.shape", Y.shape)

    print("\n now we impose W,H of output")
    H_out_imposed,W_out_imposed=13,14
    spectral_conv = SpectralConv2d(
        in_channels,
        out_channels,
        (modes1, modes2),
        first_channel_unchanged=False,
        HW_out_imposed=(H_out_imposed,W_out_imposed),
        freq_mix_size=0,
        channel_last=True)

    Y = spectral_conv.call(X)
    assert Y.shape == (batch_size, H_out_imposed, W_out_imposed,out_channels), f"Y.shape:{Y.shape} must be equal to (batch_size,H,W,out_channels):{(batch_size, H_out_imposed, W_out_imposed, out_channels)} "
    print("X.shape", X.shape)
    print("Y.shape", Y.shape)



    print("\n now we work on NCWH format")
    X = tf.ones([batch_size,in_channels,nx,ny])
    H_out_imposed, W_out_imposed = 13, 14
    spectral_conv = SpectralConv2d(in_channels, out_channels, (modes1, modes2), first_channel_unchanged=False,
        HW_out_imposed=(H_out_imposed, W_out_imposed), freq_mix_size=0, channel_last=False)
    Y = spectral_conv.call(X)
    assert Y.shape == (batch_size, out_channels,H_out_imposed, W_out_imposed), f"Y.shape:{Y.shape} must be equal to (batch_size,out_channels,H,W):{(batch_size, out_channels, H_out_imposed, W_out_imposed)} "
    print("X.shape", X.shape)
    print("Y.shape", Y.shape)
